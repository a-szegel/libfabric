...
Peer notification and remote-side cleanup on MR abort apply to send and tagged
operations only. Device RMA operations that reference a closed MR need no peer
cleanup: the initiator receives a local error completion and no provider state
exists on the peer. Emulated RMA and atomic operations are not supported:
closing an MR referenced by an in-flight emulated operation leads to undefined
behavior.
