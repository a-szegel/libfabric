...
struct efa_env efa_env = {
	...
	.rdm_max_txe = EFA_RDM_DEFAULT_TXE_POOL_SIZE,
...
void efa_env_param_get(void)
{
	...
	fi_param_get_size_t(&efa_prov, "rdm_max_txe", &efa_env.rdm_max_txe);
...
void efa_env_define()
{
	...
	fi_param_define(&efa_prov, "rdm_max_txe", FI_PARAM_SIZE_T,
			"Maximum number of concurrent efa-rdm tx operations. A value that cannot back the endpoint's tx size, or that exceeds what a txe id can index, is adjusted with a warning. (Default: %zu)",
			efa_env.rdm_max_txe);
	fi_param_define(
