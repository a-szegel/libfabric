...
struct efa_env efa_env = {
	...
	.rdm_max_txe = EFA_RDM_DEFAULT_MAX_TXE,
...
void efa_env_define()
{
	...
	fi_param_define(&efa_prov, "rdm_max_txe", FI_PARAM_SIZE_T,
			"Maximum number of concurrent efa-rdm tx operations. Endpoint creation fails if the value is 0 or exceeds what a txe id can index. (Default: %zu)",
			efa_env.rdm_max_txe);
	fi_param_define(
