...
void efa_rdm_pke_handle_tx_error(struct efa_rdm_pke *pkt_entry, int prov_errno)
{
	...
	case EFA_RDM_RXE:
		if (prov_errno == EFA_IO_COMP_STATUS_REMOTE_ERROR_RNR) {
			 ...
			 * (rather than leak the internal errno to the user CQ)
			 * and notify the sender so it can reap its txe; the
			 * clean error and rxe release are deferred to WR drain
			 * (sibling READs may still be DMAing into the buffer).
			 *
			 * Reached only on the first failure -- this branch
			 * follows the PEER_ABORT_PENDING drain-only branch
			 * above, and mark/emit set the flag -- so they run at
			 * most once per rxe, asserted below.
			 */
...
void efa_rdm_pke_handle_send_completion(struct efa_rdm_pke *pkt_entry)
{
	...
	case EFA_RDM_CTS_PKT:
		 ...
		 * released by efa_rdm_pke_handle_cts_send_completion (when
		 * cts_ready_for_release), so reading the flag afterward would
		 * be a use-after-free. A PEER_ABORT_PENDING rxe is never
		 * cts_ready_for_release (recv never completed), so it survives
		 * the handler and the drain below frees it.
		 */
	...
	case EFA_RDM_CTSDATA_PKT: {
		/*
		 * Snapshot PENDING before the handler: on a healthy full
		 * completion the handler releases the txe, so the flag must
		 * be read first. PENDING is only ever set on an aborting
		 * (OPE_ERR) txe the handler will NOT release.
		 */
		peer_abort_pending = pkt_entry->ope &&
			(pkt_entry->ope->internal_flags & EFA_RDM_OPE_PEER_ABORT_PENDING);
		efa_rdm_pke_handle_ctsdata_send_completion(pkt_entry);
		/*
		 * A successful CTSDATA WR of a LONGCTS transfer aborting on
		 * source-MR cancel: the txe was not released by the handler
		 * (bytes_acked never reaches total_len). If this is the last
		 * WR to drain, free the errored txe.
		 */
		if (peer_abort_pending)
	...
	case EFA_RDM_PEER_ERROR_PKT:
		/*
		 * PEER_ERROR_PKT send completed; both helpers are
		 * drain-gated (an rxe/txe may still have sibling WRs
