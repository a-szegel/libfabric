...
void efa_rdm_pke_handle_send_completion(struct efa_rdm_pke *pkt_entry)
{
	...
	case EFA_RDM_MEDIUM_TAGRTM_PKT:
		/*
		 * Snapshot PENDING before the handler: a healthy final segment
		 * frees the txe inside handle_medium_rtm_send_completion (read
		 * after = UAF). PENDING is only set on an aborting txe, which
		 * the handler won't free (bytes_acked < total_len because
		 * sibling WRs failed), so it survives -- and the retry below
		 * drives its deferred PEER_ERROR_PKT emit/release now that this
		 * WR has drained. A healthy full-success transfer (txe already
		 * released by the handler) is never touched.
		 */
		peer_abort_pending = pkt_entry->ope &&
			(pkt_entry->ope->internal_flags & EFA_RDM_OPE_PEER_ABORT_PENDING);
	...
	case EFA_RDM_RUNTREAD_TAGRTM_PKT:
		/*
		 * Snapshot PENDING before the handler: a healthy final runt WR
		 * of a runt-only transfer frees the txe inside
		 * handle_runtread_rtm_send_completion (read after = UAF).
		 * PENDING is only set on an aborting txe, which the handler
		 * won't free, so it survives; the retry below drives its
		 * deferred PEER_ERROR_PKT emit/release now that this WR has
		 * drained. A healthy transfer is never touched.
		 */
		peer_abort_pending = pkt_entry->ope &&
			(pkt_entry->ope->internal_flags & EFA_RDM_OPE_PEER_ABORT_PENDING);
