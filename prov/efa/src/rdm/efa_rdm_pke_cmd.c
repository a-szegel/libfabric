...
void efa_rdm_pke_handle_tx_error(struct efa_rdm_pke *pkt_entry, int prov_errno)
{
	...
	case EFA_RDM_RXE:
		if (prov_errno == EFA_IO_COMP_STATUS_REMOTE_ERROR_RNR) {
			 ...
			 * reap its txe. Both helpers are idempotent
			 * against re-entry on the same rxe; release of
			 * the rxe is gated on every WR that uses the rxe
			 * as wr_id (sibling READ contexts plus any
			 * PEER_ERROR_PKT) having drained.
			 *
