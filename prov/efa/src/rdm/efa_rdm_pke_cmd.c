...
void efa_rdm_pke_handle_tx_error(struct efa_rdm_pke *pkt_entry, int prov_errno)
{
	...
	case EFA_RDM_RXE:
		if (prov_errno == EFA_IO_COMP_STATUS_REMOTE_ERROR_RNR) {
			 ...
			 * mid-protocol. Mark the rxe peer-aborted instead
			 * of surfacing an internal-protocol error on the
			 * user's RX CQ, then notify the sender so it can
			 * reap its txe. The clean RX error completion
			 * (FI_ECANCELED / FI_EFA_ERR_PEER_ABORTED) and the
			 * rxe release are deferred to WR drain, since sibling
			 * READ WRs may still be DMAing into the recv buffer.
			 *
			 * This branch follows the PEER_ABORT_HANDLED
			 * branch above, so it is only reached on the
			 * first failure (HANDLED unset). mark/emit set
			 * HANDLED, diverting every later sibling failure
			 * to the drain-only branch -- so mark/emit run
...
void efa_rdm_pke_handle_send_completion(struct efa_rdm_pke *pkt_entry)
{
	...
	case EFA_RDM_CTSDATA_PKT: {
		 ...
		 */
		bool peer_abort_pending = pkt_entry->ope &&
			(pkt_entry->ope->internal_flags & EFA_RDM_TXE_PEER_ABORT_PENDING);
		struct efa_rdm_ope *ctsdata_txe = pkt_entry->ope;
		...
		if (peer_abort_pending)
			efa_rdm_txe_progress_peer_abort_if_drained(ctsdata_txe);
		break;
	}
	case EFA_RDM_READRSP_PKT:
