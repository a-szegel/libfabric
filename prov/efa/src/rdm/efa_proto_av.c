...
static inline struct efa_proto_av_entry *
efa_proto_av_reverse_lookup_rdm_entry(struct efa_cur_reverse_av **cur_reverse_av,
					  ...
					  struct efa_rdm_pke *pkt_entry)
{
	...
	entry = efa_proto_av_reverse_lookup_rdm_entry(
	...
	bool insert_implicit_av)
{
	...
	if (insert_shm_av) {
		err = efa_proto_av_entry_rdm_insert_shm_av(av, entry);
		if (err) {
			errno = -err;
			goto err_release;
		}
...
static inline int efa_proto_av_is_valid_address(struct efa_ep_addr *addr)
{
