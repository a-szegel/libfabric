 ...
 * retire the last such WR. The two phases key on EFA_RDM_PEER_ERROR_EMITTED:
...
void efa_rdm_txe_progress_peer_abort_if_drained(struct efa_rdm_ope *txe)
{
	...
	if (txe->internal_flags & EFA_RDM_PEER_ERROR_EMITTED) {
		/* The PEER_ERROR_PKT (if any) has drained, so surface the one
		 * withheld completion now and free the txe. Withholding until
		 * here keeps a manual-progress app polling long enough to drive
