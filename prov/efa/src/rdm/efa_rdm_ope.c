 ...
 *
 * Idempotent (first failure wins); a long-read fails many sibling READ
 * WRs. The device prov_errno is logged here for diagnostics; the user
 * sees the dedicated FI_ECANCELED / FI_EFA_ERR_PEER_ABORTED.
 *
 ...
 * DMAing into the recv buffer -- the RX completion is what authorizes
 * the app to reuse it (fi_msg(3)), and freeing the rxe with a WR
 * outstanding is a use-after-free.
 *
 * Acts only once all hold: EFA_RDM_OPE_PEER_ABORT_PENDING set (mark
 * ran), efa_outstanding_tx_ops == 0 (no WR uses it as wr_id), and no
 * queued pkt. It then writes the clean FI_ECANCELED /
 * FI_EFA_ERR_PEER_ABORTED completion (not the raw device errno),
 * returns peer_rxe to the SRX (firing the FI_MULTI_RECV release entry
 * for a multi-recv child), and frees the rxe.
 *
 * Idempotent; called from every site that can retire the last WR.
 */
void efa_rdm_rxe_release_peer_abort_if_drained(struct efa_rdm_ope *rxe)
{
	...
	/* Drain complete: report the clean, dedicated peer-abort error to
	 * the app (the raw device status was logged at mark time, and
	 * handle_error intentionally does not free the rxe), then release
	 * it -- efa_rdm_rxe_release returns the SRX entry (firing the
	 * FI_MULTI_RECV release for a multi-recv child, fi_msg(3)) and
	 * frees the rxe. */
...
void efa_rdm_rxe_emit_peer_error(struct efa_rdm_ope *rxe, int prov_errno)
{
	...
	/* Post PEER_ERROR_PKT. The peer-abort machinery owns rxe
	 * release: the drain helper frees the rxe once all WRs (sibling
	 * READ WRs and this PEER_ERROR_PKT) have drained. */
 ...
 *        cleanup has drained.
 *
 * No-op until the txe is marked EFA_RDM_OPE_PEER_ABORT_PENDING and
 * every WR using it as wr_id has drained (efa_outstanding_tx_ops == 0,
 * no queued pkt). Called from every send-completion / tx-error site
...
void efa_rdm_txe_handle_error(struct efa_rdm_ope *txe, int err, int prov_errno)
{
	 ...
	 *
	 * If the user canceled the source MR mid-CTSDATA -- detected
	 * pre-post by the MR generation check (-FI_ECANCELED) or post-post
	 * by the NIC (LOCAL_ERROR_INVALID_LKEY) -- and the peer supports
	 * the feature, tell the receiver so its matched recv completes with
	 * an error instead of hanging.
	 *
	 * The gate keys on prev_state == EFA_RDM_OPE_SEND (not bytes_sent):
	 * the packet carries op_id = txe->rx_id, which efa_rdm_pke_handle_cts_recv()
	 * sets at the same time it moves the txe to OPE_SEND, so any other
	 * state has a stale rx_id. The pre-post FI_ECANCELED path only
	 * iterates ope_longcts_send_list (OPE_SEND txes), so both detection
	 * paths are covered.
	 *
	 * We do NOT emit here: sibling CTSDATA WRs (still using the txe as
	 * wr_id) are in flight. Mark PEER_ABORT_PENDING and defer the emit
	 * to efa_rdm_txe_progress_peer_abort_if_drained() once they drain,
	 * so no CTSDATA can race the notification (mr_abort design §5); the
	 * txe is freed when the PEER_ERROR_PKT's own completion drains.
	 */
	if (!(txe->internal_flags & EFA_RDM_OPE_INTERNAL) &&
...
int efa_rdm_ope_process_queued_ope(struct efa_rdm_ope *ope, uint32_t flag)
{
	...
	/*
	 * The MR generation check guards against reposting a WR whose SGE
	 ...
	 * a provider TX buffer and carries no user payload from ope->desc[].
	 * The PEER_ERROR_PKT is emitted *because* the source MR was just
	 * closed (gen bumped), so applying the gen check here would cancel
	 * the very notification that tells the peer to recover.
	 */
