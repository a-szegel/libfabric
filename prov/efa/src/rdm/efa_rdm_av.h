 ...
 * efa_rdm_ah adds the implicit/explicit refcount split, the
 * implicit_conn_list (entries using this AH), and the LRU list
 * entry for AH eviction.
 *
 * pahole: size: 128, cachelines: 2
 *
 * All efa_rdm_ah fields are control path only (AV insert/remove/eviction).
...
struct efa_rdm_av_entry {
	...
	struct efa_rdm_av_entry_ep_peer_map_entry *ep_peer_map; /* 56  8 */
 ...
 */
struct efa_rdm_av_entry_ep_peer_map_entry {
 ...
 * @return	pointer to the containing efa_rdm_ah
 */
static inline struct efa_rdm_ah *efa_rdm_ah_from_ah(struct efa_ah *ah)
{
	return container_of(ah, struct efa_rdm_ah, ah);
}
...
efa_rdm_av_entry_ep_addr(struct efa_rdm_av_entry *entry)
{
