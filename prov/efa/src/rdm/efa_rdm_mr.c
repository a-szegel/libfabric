...
static int efa_rdm_mr_cache_close(fid_t fid)
{
	...
	ofi_mr_cache_delete(((struct efa_rdm_domain *) efa_mr->domain)->cache, ((struct efa_rdm_mr *) efa_mr)->entry);
				     ...
				     uint64_t flags)
{
	...
	{
	struct efa_rdm_domain *rdm_domain = (struct efa_rdm_domain *) efa_mr->domain;
			       ...
			       const struct fi_mr_attr *mr_attr)
{
	...
	{
	struct efa_rdm_domain *rdm_domain = (struct efa_rdm_domain *) efa_rdm_mr->efa_mr.domain;
	if (rdm_domain->cache)
			       ...
			       struct ofi_mr_entry *entry)
{
	...
	/**
	 * EFA provider requires FI_MR_HMEM so all non-system memory must be registered
	 ...
	 * behavior has been removed (because it violates security rules) and EFA
