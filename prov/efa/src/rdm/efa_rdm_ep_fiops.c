...
static int efa_rdm_ep_bind(struct fid *ep_fid, struct fid *bfid, uint64_t flags)
{
	...
	case FI_CLASS_AV:
		...
		if (efa_rdm_ep->shm_ep) {
			...
			assert(proto_av->shm_rdm_av);
			ret = fi_ep_bind(efa_rdm_ep->shm_ep, &proto_av->shm_rdm_av->fid, flags);
			if (ret)
				return ret;
		}
		break;
...
int efa_rdm_ep_close_shm_resources(struct efa_rdm_ep *efa_rdm_ep)
{
	...
	{
		struct efa_proto_av *proto_av = container_of(efa_av, struct efa_proto_av, efa_av);
		if (proto_av->shm_rdm_av) {
			ret = fi_close(&proto_av->shm_rdm_av->fid);
			if (ret) {
				EFA_WARN(FI_LOG_EP_CTRL, "Unable to close shm av: %s\n", fi_strerror(-ret));
				retv = ret;
			}
			proto_av->shm_rdm_av = NULL;
		}
	}
