 ...
 */
static void efa_rdm_pke_peer_error_handle_rxe(struct efa_rdm_ope *rxe,
						      int prov_errno)
{
...
void efa_rdm_pke_handle_peer_error_recv(struct efa_rdm_pke *pkt_entry)
{
	...
	/*
	 * RX->TX: a data receiver's RDMA READ failed. The wire always carries
	 * the emitter's copy of our txe index (learned from the RTM). Trust it
	 * only while the slot is still allocated, still a txe, and still this
	 * transfer (msg_id match): pooled opes are freed and reused, and
	 * acting on a stale one would corrupt unrelated state.
	 * ofi_bufpool_get_ibuf() does not bounds-check, so gate on
	 * ofi_bufpool_ibuf_is_valid() first. If the hint does not resolve,
	 * the txe already resolved itself locally and the packet is stale:
	 * drop it. Never fall through to msg_id resolution -- the TX and RX
	 * msg_id spaces between two endpoints are independent counters, so
	 * treating a TX-space msg_id as an inbound-message key would corrupt
	 * the receive reorder state.
	 */
	...
	/*
	 * No matched rxe: the msg_id is in exactly one of three states -- fully
	 * processed (clean drop), buffered out-of-order (mark the buffered
	 * segment aborted), or never arrived (queue an abort marker so the
	 * reorder window slides past it). efa_rdm_peer_queue_aborted_msg_marker()
	 * handles whichever applies and consumes pkt_entry -- do not touch it
	 * afterward.
	 */
