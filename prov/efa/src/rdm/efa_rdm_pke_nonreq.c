...
void efa_rdm_pke_handle_peer_error_recv(struct efa_rdm_pke *pkt_entry)
{
	...
	case EFA_RDM_RXE:
		 ...
		 * already knows, it told us.  On a successful re-queue
		 * the recovery owns no user CQ entry, so release the rxe
		 * here. */
		if (efa_rdm_rxe_recover_from_peer_abort(ope, prov_errno))
			efa_rdm_rxe_release_internal(ope);
		break;
	default:
		EFA_WARN(FI_LOG_CQ,
