...
void efa_rdm_pke_handle_rma_read_completion(struct efa_rdm_pke *context_pkt_entry)
{
	...
	if (x_entry_type == EFA_RDM_TXE) {
		 ...
		 * completions for that rxe; the failed one(s) already
		 * marked the rxe peer-aborted (EFA_RDM_RXE_PEER_ABORT_HANDLED).
		 * For a sibling that nonetheless completes successfully, do
		 * NOT fall into the EOR / recv-completed path (it would post
		 * a spurious EOR and write a bogus success completion on top
		 * of the abort). The user error completion and the rxe free
		 * are deferred to the drain helper; if this successful
		 * sibling is the last reference, the rxe is reaped here.
		 */
 ...
 *     write a clean TX CQ error (FI_ECANCELED /
 *     FI_EFA_ERR_PEER_ABORTED), and release the txe.
 *
 ...
 *     error-dispatch site uses: mark the matched rxe peer-aborted;
 *     once it drains, a clean RX CQ error (FI_ECANCELED /
 *     FI_EFA_ERR_PEER_ABORTED) is written and the rxe is reaped.
 *
...
void efa_rdm_pke_handle_peer_error_recv(struct efa_rdm_pke *pkt_entry)
{
	...
	if (err_hdr->ref_kind == EFA_RDM_PEER_ERROR_REF_MSG_ID) {
		...
		if (ope->state == EFA_RDM_RXE_UNEXP) {
			/* No user op is bound; no CQ entry is owed. Release
			 * the buffered segments, then release the rxe.
			 * efa_rdm_rxe_release frees peer_rxe via
			 * owner_ops->free_entry (unlinks the unexpected entry
			 * + frees it) then release_internal (rxe_map removal
			 * + rxe free). */
			if (ope->unexp_pkt) {
	...
	case EFA_RDM_TXE:
		 ...
		 */
		if (OFI_LIKELY(efa_rdm_ep_domain(ep)->num_read_msg_in_flight > 0))
			efa_rdm_ep_domain(ep)->num_read_msg_in_flight -= 1;

		/* Surface the same clean, dedicated peer-abort code on the
	...
	case EFA_RDM_RXE:
		 ...
		 * Mark our matched rxe peer-aborted; the clean RX error
		 * completion (FI_ECANCELED / FI_EFA_ERR_PEER_ABORTED) and
		 * the rxe release are deferred to WR drain.
		 * We do NOT emit a PEER_ERROR_PKT back: the sender
		 * already knows, it told us.  Release the rxe via the
		 * drain helper -- a no-op if any control send (e.g.,
		 * CTS) is still outstanding on this rxe.
		 */
		efa_rdm_rxe_mark_peer_aborted(ope, prov_errno);
		efa_rdm_rxe_release_peer_abort_if_drained(ope);
		break;
