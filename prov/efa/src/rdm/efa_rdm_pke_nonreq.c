...
void efa_rdm_pke_handle_rma_read_completion(struct efa_rdm_pke *context_pkt_entry)
{
	...
	if (x_entry_type == EFA_RDM_TXE) {
		 ...
		 * of the abort). Just attempt the drain: record_tx_op_completed()
		 * already decremented efa_outstanding_tx_ops for this WR, so
		 * if this is the last reference the rxe is freed here.
		 */
...
void efa_rdm_pke_handle_peer_error_recv(struct efa_rdm_pke *pkt_entry)
{
	...
	case EFA_RDM_TXE:
		 ...
		 * RUNTREAD RTM. Guard against underflow: the counter is
		 * an unsigned domain-wide gauge, and a duplicate or
		 * misattributed packet must not wrap it (efa_rdm_peer.c
		 * gates peer cleanup on it reaching 0). */
	...
	case EFA_RDM_RXE:
		 ...
		 * Recover locally -- re-queue our rxe back into the SRX
		 * (or write a user CQ error for a multi-recv child).
