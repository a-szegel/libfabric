	...
	struct efa_rdm_pke *pkt_entry)
{
	...
	if (ep->base_ep.util_ep.caps & FI_SOURCE) {
		...
		src_addr = efa_proto_av_reverse_lookup_rdm(container_of(efa_av, struct efa_proto_av, efa_av),
			   ...
			   struct efa_ep_addr *efa_ep_addr)
{
	...
	addr = ofi_av_lookup_fi_addr(&container_of(ep->base_ep.av, struct efa_proto_av, efa_av)->util_av_implicit,
				     (void *) efa_ep_addr);
	if (addr != FI_ADDR_NOTAVAIL) {
		implicit = true;
				  ...
				  struct efa_rdm_pke *pkt_entry)
{
	 ...
	 */
	explicit_fi_addr =
		efa_proto_av_reverse_lookup_rdm(container_of(efa_av, struct efa_proto_av, efa_av), gid, qpn, pkt_entry);
	...
	implicit_fi_addr =
		efa_proto_av_reverse_lookup_rdm_implicit(container_of(efa_av, struct efa_proto_av, efa_av), gid, qpn, pkt_entry);
	 ...
	 * and restoring it
	 */
	HASH_FIND(hh, container_of(ep->base_ep.av, struct efa_proto_av, efa_av)->evicted_peers_hashset, &efa_ep_addr,
	 ...
	 */
	ret = efa_proto_av_insert_one(container_of(ep->base_ep.av, struct efa_proto_av, efa_av), &efa_ep_addr, &implicit_fi_addr,
				0, NULL, false, true);
