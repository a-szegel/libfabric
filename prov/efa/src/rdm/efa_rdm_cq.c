	...
	struct efa_rdm_pke *pkt_entry)
{
	...
	if (ep->base_ep.util_ep.caps & FI_SOURCE) {
		...
		src_addr = efa_rdm_av_reverse_lookup(ep->rdm_av,
			   ...
			   struct efa_ep_addr *efa_ep_addr)
{
	...
	addr = ofi_av_lookup_fi_addr(&ep->rdm_av->util_av_implicit,
				     (void *) efa_ep_addr);
	if (addr != FI_ADDR_NOTAVAIL) {
		implicit = true;
				  ...
				  struct efa_rdm_pke *pkt_entry)
{
	 ...
	 */
	explicit_fi_addr =
		efa_rdm_av_reverse_lookup(ep->rdm_av, gid, qpn, pkt_entry);
	...
	implicit_fi_addr =
		efa_rdm_av_reverse_lookup_implicit(ep->rdm_av, gid, qpn, pkt_entry);
	 ...
	 * and restoring it
	 */
	HASH_FIND(hh, ep->rdm_av->evicted_peers_hashset, &efa_ep_addr,
	 ...
	 */
	ret = efa_rdm_av_insert_one(ep->rdm_av, &efa_ep_addr, &implicit_fi_addr,
				0, NULL, false, true);
