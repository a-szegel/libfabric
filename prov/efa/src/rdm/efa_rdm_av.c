...
/*
 * During the coexistence period between efa_conn and efa_rdm_av_entry,
 * efa_av_reverse_av_add / _remove read av_entry->conn.ah->ahn on a
 * pointer that may actually be an efa_rdm_av_entry *. This relies on
 ...
 * with struct efa_ah *ah) and struct efa_rdm_av_entry (directly).
 * If either struct is ever reordered, these static asserts break loudly
 * instead of silently reading the wrong field.
 */
_Static_assert(offsetof(struct efa_rdm_av_entry, ep_addr) ==
	       offsetof(struct efa_av_entry, ep_addr),
	       "efa_av_entry and efa_rdm_av_entry must share ep_addr offset");
_Static_assert(offsetof(struct efa_rdm_av_entry, ah) ==
	       offsetof(struct efa_av_entry, conn) + offsetof(struct efa_conn, ah),
	       "efa_av_entry->conn.ah and efa_rdm_av_entry->ah must be at the same offset");
...
static void efa_rdm_ah_lru_move(struct efa_domain *domain, struct efa_ah *ah);
...
efa_rdm_av_addr_to_entry_impl(struct util_av *util_av, fi_addr_t fi_addr)
{
	struct util_av_entry *util_av_entry;
	struct efa_rdm_av_entry *entry;
	...
	entry = (struct efa_rdm_av_entry *)util_av_entry->data;
 ...
 * @return	pointer to entry if valid, NULL otherwise
 */
struct efa_rdm_av_entry *efa_rdm_av_addr_to_entry(struct efa_rdm_av *av,
						      fi_addr_t fi_addr)
{
	return efa_rdm_av_addr_to_entry_impl(&av->efa_av.util_av, fi_addr);
}
 ...
 * @return	pointer to entry if valid, NULL otherwise
 */
struct efa_rdm_av_entry *efa_rdm_av_addr_to_entry_implicit(
	struct efa_rdm_av *av, fi_addr_t fi_addr)
{
	return efa_rdm_av_addr_to_entry_impl(&av->util_av_implicit, fi_addr);
}
 ...
 */
void efa_rdm_av_entry_ep_peer_map_insert(
	struct efa_rdm_av_entry *entry,
	struct efa_rdm_av_entry_ep_peer_map_entry *map_entry)
{
 ...
 */
struct efa_rdm_peer *efa_rdm_av_entry_ep_peer_map_lookup(
	struct efa_rdm_av_entry *entry, struct efa_rdm_ep *ep)
{
	struct efa_rdm_av_entry_ep_peer_map_entry *map_entry;
 ...
 */
void efa_rdm_av_entry_ep_peer_map_remove(
	struct efa_rdm_av_entry *entry, struct efa_rdm_ep *ep)
{
	struct efa_rdm_av_entry_ep_peer_map_entry *map_entry;
 ...
 */
int efa_rdm_av_entry_insert_shm_av(struct efa_rdm_av *av,
					  struct efa_rdm_av_entry *entry)
{
	int err, ret;
	char smr_name[EFA_SHM_NAME_MAX];
	size_t smr_name_len;
	struct efa_ep_addr *ep_addr = efa_rdm_av_entry_ep_addr(entry);
 ...
 * @param[in]	entry	proto av entry
 */
void efa_rdm_av_entry_deinit(struct efa_rdm_av *av,
				   struct efa_rdm_av_entry *entry)
{
	int err;
	struct efa_rdm_av_entry_ep_peer_map_entry *peer_map_entry, *tmp;
 ...
 */
static inline int efa_rdm_av_implicit_av_lru_insert(struct efa_rdm_av *av,
						      struct efa_rdm_av_entry *entry)
{
	size_t cur_size;
	struct efa_ep_addr_hashable *ep_addr_hashable;
	struct efa_rdm_av_entry *entry_to_release;
	 ...
	 * old neighbors. Re-init so that efa_rdm_av_entry_release's call to
	...
	EFA_INFO(FI_LOG_AV,
		 ...
		 efa_rdm_av_entry_ep_addr(entry_to_release)->qpn,
		 efa_rdm_av_entry_ep_addr(entry_to_release)->qkey);
	...
	assert(ofi_genlock_held(&av->efa_av.domain->srx_lock));
	efa_rdm_av_entry_release(av, entry_to_release, true);
 ...
 */
void efa_rdm_av_implicit_av_lru_entry_move(struct efa_rdm_av *av,
					     struct efa_rdm_av_entry *entry)
{
	assert(ofi_genlock_held(&av->efa_av.domain->srx_lock));
	...
	efa_rdm_ah_lru_move(av->efa_av.domain, entry->ah);
}

/* ---- Reverse lookup (RDM, connid-aware) ---- */
				      ...
				      struct efa_rdm_pke *pkt_entry)
{
	 ...
	 * of a efa_rdm_av_entry which has the same layout prefix.
	 */
	if (!pkt_entry) {
		return (struct efa_rdm_av_entry *)cur_entry->av_entry;
	}
	...
	return OFI_LIKELY(!!prv_entry) ? (struct efa_rdm_av_entry *)prv_entry->av_entry : NULL;
}
 ...
 */
fi_addr_t efa_rdm_av_reverse_lookup(struct efa_rdm_av *av,
					  uint16_t ahn, uint16_t qpn,
					  struct efa_rdm_pke *pkt_entry)
{
	struct efa_rdm_av_entry *entry;
 ...
 */
fi_addr_t efa_rdm_av_reverse_lookup_implicit(struct efa_rdm_av *av,
						   uint16_t ahn, uint16_t qpn,
						   struct efa_rdm_pke *pkt_entry)
{
	struct efa_rdm_av_entry *entry;
	...
	if (OFI_LIKELY(!!entry)) {
		efa_rdm_av_implicit_av_lru_entry_move(av, entry);
 ...
 */
static void efa_rdm_av_entry_release_reverse_av(struct efa_rdm_av *av,
						  struct efa_rdm_av_entry *entry,
 ...
 * @param[in]	release_from_implicit_av	whether entry is in implicit AV
 */
static void efa_rdm_av_entry_release_util_av(struct efa_rdm_av *av,
					       struct efa_rdm_av_entry *entry,
					       bool release_from_implicit_av)
{
	...
	inet_ntop(AF_INET6, efa_rdm_av_entry_ep_addr(entry)->raw, gidstr, INET6_ADDRSTRLEN);
	EFA_INFO(FI_LOG_AV, "efa_rdm_av_entry released! entry[%p] GID[%s] QP[%u]\n",
		 entry, gidstr, efa_rdm_av_entry_ep_addr(entry)->qpn);
 ...
 * @param[in]	release_from_implicit_av	whether entry is in implicit AV
 */
void efa_rdm_av_entry_release(struct efa_rdm_av *av,
				struct efa_rdm_av_entry *entry,
				bool release_from_implicit_av)
{
	assert(ofi_genlock_held(&av->efa_av.domain->srx_lock));

	efa_rdm_av_entry_release_reverse_av(av, entry, release_from_implicit_av);
	efa_rdm_av_entry_deinit(av, entry);
	...
	efa_rdm_ah_release(av->efa_av.domain, entry->ah, release_from_implicit_av);
	efa_rdm_av_entry_release_util_av(av, entry, release_from_implicit_av);
 ...
 * @param[in]	release_from_implicit_av	whether entry is in implicit AV
 */
void efa_rdm_av_entry_release_ah_unsafe(struct efa_rdm_av *av,
					  struct efa_rdm_av_entry *entry,
					  bool release_from_implicit_av)
{
	assert(ofi_genlock_held(&av->efa_av.domain->srx_lock));
	...
	release_from_implicit_av ? efa_rdm_ah_from_ah(entry->ah)->implicit_refcnt-- :
				   efa_rdm_ah_from_ah(entry->ah)->explicit_refcnt--;
	entry->ah->refcnt--;
 ...
 * @param[in]	ah	base AH (must be embedded in efa_rdm_ah)
 */
static void efa_rdm_ah_lru_move(struct efa_domain *domain, struct efa_ah *ah)
{
	struct efa_rdm_ah *rdm_ah = efa_rdm_ah_from_ah(ah);

	assert(efa_rdm_ah_from_ah(ah)->implicit_refcnt > 0 || efa_rdm_ah_from_ah(ah)->explicit_refcnt > 0);
	assert(dlist_entry_in_list(&domain->ah_lru_list,
				   &rdm_ah->lru_list_entry));

	dlist_remove(&rdm_ah->lru_list_entry);
	dlist_insert_tail(&rdm_ah->lru_list_entry, &domain->ah_lru_list);
}
 ...
 */
static int efa_rdm_ah_evict(struct efa_domain *domain)
{
	struct efa_rdm_av_entry *entry_to_release;
	...
	dlist_foreach_container(&domain->ah_lru_list, struct efa_rdm_ah,
				rdm_ah_tmp, lru_list_entry) {
	...
	dlist_foreach_container_safe(&rdm_ah_to_release->implicit_conn_list,
		...
		efa_rdm_av_entry_release_ah_unsafe(entry_to_release->av,
						     entry_to_release, true);
	}

	if (rdm_ah_to_release->implicit_refcnt == 0 &&
	    rdm_ah_to_release->explicit_refcnt == 0) {
		dlist_remove(&rdm_ah_to_release->lru_list_entry);
		assert(dlist_empty(&rdm_ah_to_release->implicit_conn_list));
		assert(rdm_ah_to_release->ah.refcnt == 0);
		efa_ah_destroy(domain, &rdm_ah_to_release->ah);
	}
 ...
 *
 * Calls efa_ah_alloc with sizeof(efa_rdm_ah) to allocate the
 ...
 * field mutations to serialize against concurrent efa_rdm_ah_alloc
 * / efa_rdm_ah_release on a different AV.
 *
 * @param[in]	domain		efa domain
 * @param[in]	gid		GID
					 ...
					 bool insert_implicit_av)
{
	 ...
	 * concurrent efa_rdm_ah_release could have dropped both proto
	...
	rdm_ah = efa_rdm_ah_from_ah(ah);

	/*
	 * first_rdm_user is true when both proto refcnts are zero — either
	 ...
	 * into the LRU list.
	 */
	first_rdm_user = (rdm_ah->implicit_refcnt == 0 &&
			    rdm_ah->explicit_refcnt == 0);
	if (first_rdm_user) {
		dlist_init(&rdm_ah->implicit_conn_list);
		dlist_insert_tail(&rdm_ah->lru_list_entry,
				  &domain->ah_lru_list);
	}
				 ...
				 bool release_from_implicit_av)
{
	...
	if (rdm_ah->implicit_refcnt == 0 && rdm_ah->explicit_refcnt == 0) {
		dlist_remove(&rdm_ah->lru_list_entry);
 ...
 * efa_rdm_av_entry_deinit on the error path, which walks the per-entry
 ...
 */
struct efa_rdm_av_entry *efa_rdm_av_entry_alloc(
	struct efa_rdm_av *av, struct efa_ep_addr *raw_addr,
	...
	bool insert_implicit_av)
{
	...
	if (insert_implicit_av) {
		...
		err = efa_rdm_av_implicit_av_lru_insert(av, entry);
		if (err)
			goto err_release;
		on_lru_list = true;
	} else {
	...
	entry->ah = efa_rdm_ah_alloc(av->efa_av.domain, raw_addr->raw, insert_implicit_av);
	if (!entry->ah)
		goto err_release;

	if (insert_implicit_av)
		dlist_insert_tail(&entry->ah_implicit_conn_list_entry,
				  &efa_rdm_ah_from_ah(entry->ah)->implicit_conn_list);
	 ...
	 */
	if (insert_shm_av) {
		err = efa_rdm_av_entry_insert_shm_av(av, entry);
		if (err) {
			errno = -err;
			goto err_release;
		}
...
err_release:
	...
	if (entry->ah)
		efa_rdm_ah_release(av->efa_av.domain, entry->ah, insert_implicit_av);
 ...
 */
int efa_rdm_av_entry_implicit_to_explicit(struct efa_rdm_av *av,
					    ...
					    fi_addr_t *fi_addr)
{
	int err;
	struct efa_ah *ah;
	struct efa_rdm_av_entry *implicit_entry, *explicit_entry;
	struct efa_rdm_ep *ep;
	struct dlist_entry *list_entry;
	struct util_av_entry *implicit_util_av_entry, *explicit_util_av_entry;
	struct efa_rdm_av_entry_ep_peer_map_entry *map_entry, *tmp;
	...
	implicit_entry = (struct efa_rdm_av_entry *) implicit_util_av_entry->data;
	...
	explicit_entry = (struct efa_rdm_av_entry *) explicit_util_av_entry->data;
	assert(efa_is_same_addr(
	...
	assert(!dlist_empty(&efa_rdm_ah_from_ah(ah)->implicit_conn_list));
	dlist_remove(&implicit_entry->ah_implicit_conn_list_entry);
	efa_rdm_ah_lru_move(av->efa_av.domain, ah);
	efa_rdm_ah_from_ah(ah)->implicit_refcnt--;
	efa_rdm_ah_from_ah(ah)->explicit_refcnt++;
 ...
 * @return	0 on success, negative error code on failure
 */
int efa_rdm_av_insert_one(struct efa_rdm_av *av, struct efa_ep_addr *addr,
			    ...
			    bool insert_shm_av, bool insert_implicit_av)
{
	...
	if (implicit_fi_addr != FI_ADDR_NOTAVAIL) {
		...
		if (insert_implicit_av) {
			entry = efa_rdm_av_addr_to_entry_implicit(av, implicit_fi_addr);
			efa_rdm_av_implicit_av_lru_entry_move(av, entry);
			*fi_addr = implicit_fi_addr;
			goto out;
		}

		ret = efa_rdm_av_entry_implicit_to_explicit(av, addr, implicit_fi_addr, fi_addr);
		if (ret)
			*fi_addr = FI_ADDR_NOTAVAIL;
		goto out;
	}

	entry = efa_rdm_av_entry_alloc(av, addr, flags, context, insert_shm_av, insert_implicit_av);
	if (!entry) {
		*fi_addr = FI_ADDR_NOTAVAIL;
			       ...
			       uint64_t flags, void *context)
{
	...
	struct efa_rdm_av *av = container_of(base_av, struct efa_rdm_av, efa_av);
	...
	for (i = 0; i < count; i++) {
		...
		ret = efa_rdm_av_insert_one(av, addr_i, &fi_addr_res, flags, context, true, false);
		if (ret) {
 ...
 * @return	0 on success, negative error code on failure
 */
static int efa_rdm_av_lookup(struct fid_av *av_fid, fi_addr_t fi_addr,
			       void *addr, size_t *addrlen)
{
	struct efa_av *base_av = container_of(av_fid, struct efa_av, util_av.av_fid);
	struct efa_rdm_av *av = container_of(base_av, struct efa_rdm_av, efa_av);
	struct efa_rdm_av_entry *entry = NULL;
	...
	ofi_genlock_lock(&av->efa_av.util_av.lock);
	entry = efa_rdm_av_addr_to_entry(av, fi_addr);
	if (!entry) {
		ofi_genlock_unlock(&av->efa_av.util_av.lock);
		return -FI_EINVAL;
	}
			       ...
			       size_t count, uint64_t flags)
{
	...
	av = container_of(base_av, struct efa_rdm_av, efa_av);
	if (av->efa_av.type != FI_AV_TABLE)
		return -FI_EINVAL;
	...
	ofi_genlock_lock(&av->efa_av.util_av.lock);
	for (i = 0; i < count; i++) {
		entry = efa_rdm_av_addr_to_entry(av, fi_addr[i]);
		if (!entry) {
			err = -FI_EINVAL;
			break;
		}

		efa_rdm_av_entry_release(av, entry, false);
	}
 ...
 */
static const char *efa_rdm_av_straddr(struct fid_av *av_fid, const void *addr,
					char *buf, size_t *len)
{
...
static struct fi_ops_av efa_rdm_av_ops = {
	...
	.lookup = efa_rdm_av_lookup,
	.straddr = efa_rdm_av_straddr
};
 ...
 * @param[in]	av	protocol AV
 */
static void efa_rdm_av_close_reverse_av(struct efa_rdm_av *av)
{
	...
	HASH_ITER(hh, av->efa_av.cur_reverse_av, cur_entry, curtmp) {
		efa_rdm_av_entry_release(av, (struct efa_rdm_av_entry *)cur_entry->av_entry, false);
	}

	HASH_ITER(hh, av->efa_av.prv_reverse_av, prv_entry, prvtmp) {
		efa_rdm_av_entry_release(av, (struct efa_rdm_av_entry *)prv_entry->av_entry, false);
	}
	...
	HASH_ITER(hh, av->cur_reverse_av_implicit, cur_entry, curtmp) {
		efa_rdm_av_entry_release(av, (struct efa_rdm_av_entry *)cur_entry->av_entry, true);
	}

	HASH_ITER(hh, av->prv_reverse_av_implicit, prv_entry, prvtmp) {
		efa_rdm_av_entry_release(av, (struct efa_rdm_av_entry *)prv_entry->av_entry, true);
	}
...
static int efa_rdm_av_close(struct fid *fid)
{
	...
	efa_rdm_av_close_reverse_av(av);
		      ...
		      struct fid_av **av_fid, void *context)
{
	...
	context_len = sizeof(struct efa_rdm_av_entry) - EFA_EP_ADDR_LEN;
