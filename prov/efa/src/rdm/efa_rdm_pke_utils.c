...
int efa_rdm_ep_flush_queued_blocking_copy_to_hmem(struct efa_rdm_ep *ep)
{
	...
	for (i = 0; i < ep->queued_copy_num; ++i) {
		...
		if (desc->flags & OFI_HMEM_DATA_DEV_REG_HANDLE) {
			...
			bytes_copied[i] = ofi_copy_to_hmem_iov(desc->efa_mr.iface,
			                                       desc->device,
			                                       rxe->iov, rxe->iov_count,
			                                       segment_offset + ep->msg_prefix_size,
			                                       data, pkt_entry->payload_size);
					   ...
					   bool *restrict gdrcopy_available)
{
	...
	*gdrcopy_available = efa_rdm_mr->flags & OFI_HMEM_DATA_DEV_REG_HANDLE;
...
uint32_t *efa_rdm_pke_connid_ptr(struct efa_rdm_pke *pkt_entry)
{
	...
	case EFA_RDM_PEER_ERROR_PKT:
		return &efa_rdm_pke_get_peer_error_hdr(pkt_entry)->connid;
