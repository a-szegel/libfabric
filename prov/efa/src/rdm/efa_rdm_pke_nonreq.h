 ...
 *        in-flight protocol step (e.g. closed an MR mid-protocol or
 *        tore down its endpoint), as opposed to a genuine local/network
 *        fault the user must still see.
 *
 * Recognized statuses:
 *  - REMOTE_ERROR_BAD_ADDRESS (7): sender's MR invalid/deregistered
 *    under a receiver-initiated RDMA READ.
 ...
 *        msg_id (REF_MSG_ID) rather than a shared ope index.
 *
 ...
 * only shared id): medium RTM, and runt-only runtread
 * (bytes_runt == total_len, all data in REQ packets, no RDMA READ).
 */
static inline
bool efa_rdm_txe_peer_abort_uses_msg_id(struct efa_rdm_ope *txe)
{
	return efa_rdm_pkt_type_is_medium(txe->protocol) ||
