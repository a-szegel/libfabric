					     ...
					     uint32_t msg_id)
{
	...
	if (OFI_UNLIKELY(!ooo_entry)) {
		 ...
		 * avoid leaking an rx pkt on the abort path. (The
		 * !rx_copy_ooo branch of get_ooo_pke returns pkt_entry itself
		 * and never NULL, so this only runs on a true clone failure.) */
		efa_rdm_pke_release_rx(pkt_entry);
		return -FI_ENOMEM;
	}
	 ...
	 *
	 * EFA_RDM_REQ_MSG is stamped even when the aborted message was
	 * tagged: the flag exists only to satisfy get_rtm_msg_id()'s
	 * "is this a msg/atomic req" assert during slotting / overflow
	 * promotion. The tombstone is never matched against a posted recv
	 * and never processed as an RTM -- the drain loop's
	 * EFA_RDM_PKE_ABORTED check (which runs before reading the msg_id)
	 * releases it and slides the window -- so the msg-vs-tagged
