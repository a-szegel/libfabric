...
bool efa_rdm_peer_abort_ooo_msg(struct efa_rdm_peer *peer, uint32_t msg_id)
{
	...
	/* 2. Check recvwin (abort marker, do not NULL the slot). */
	if (ofi_recvwin_id_valid(&peer->robuf, msg_id)) {
 ...
 *        window as an abort marker, so the window can advance past a
 ...
 *  2. A segment was already buffered for this msg_id: mark it aborted in
 *     place (efa_rdm_peer_abort_ooo_msg); this control packet is not
 *     needed.
 *  3. Never arrived: hold THIS packet, stamp it as the abort marker,
 ...
 * @return 1 if the packet was queued as an abort marker, 0 if it was not
