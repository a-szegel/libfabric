 ...
 *
 * TX_TO_RX: A sender aborted an in-flight send. Any two-sided protocol can
 *           take this path (a queued RTM canceled by the MR-gen check).
 * 			 The packet's receiver is the transfer's receiver.
 * RX_TO_TX: A receiver's RDMA READ failed (LONGREAD / RUNTREAD).
 *           The packet's receiver is the transfer's sender.
 */
