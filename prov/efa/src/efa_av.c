...
/* Stub: this function moved to efa_rdm_av.c as efa_rdm_ah_lru_move.
				    ...
				    uint16_t qpn, struct efa_rdm_pke *pkt_entry)
{
	...
	return efa_rdm_av_reverse_lookup(rdm_av, ahn, qpn, pkt_entry);
}
					     ...
					     struct efa_rdm_pke *pkt_entry)
{
	...
	return efa_rdm_av_reverse_lookup_implicit(rdm_av, ahn, qpn, pkt_entry);
}
					 ...
					 fi_addr_t *fi_addr)
{
	...
	HASH_ITER(hh, implicit_conn->ep_peer_map, map_entry, tmp) {
		...
		map_entry->peer.av_entry = (struct efa_rdm_av_entry *)explicit_conn;
	}
