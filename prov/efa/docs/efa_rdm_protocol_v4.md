...
| `p2p_supported`  | 1                     | integer       | `uint8_t`    | `HANDSHAKE_HMEM_P2P_HDR`     |
| _reserved_       | 7                     | _N/A_         | _N/A_        | `HANDSHAKE_HMEM_P2P_HDR`     |
...
- `p2p_supported` indicates whether the peer's accelerator memory buffers can be accessed by the NIC directly (peer-to-peer). This applies to all operations posted to the NIC, including RDMA read/write sources and targets. A value of 1 means the peer only uses system memory or has accelerator memory with p2p support, so NIC-direct operations are safe. A value of 0 means the peer has accelerator memory (FI_HMEM) without p2p support, so operations posted directly to the NIC involving that peer's buffers may fail. Legacy peers that do not include this header are assumed to have p2p support (they predate FI_HMEM support).

The `p2p_supported` field is part of an 8-byte HMEM capability header block. Currently only the first byte is defined (`p2p_supported`); the remaining 7 bytes are reserved. To extend HMEM capability information in the future, new fields can be appended within this same 8-byte block, each gated by a new `HANDSHAKE_HMEM_*_HDR` flag bit. For example, a future `HANDSHAKE_HMEM_FOO_HDR` flag (bit 4) would indicate that the next field(s) in the block are valid. The sender sets all applicable `HMEM_*` flag bits and populates the corresponding fields; the receiver checks each flag independently to determine which fields to interpret. Flags are cumulative — a sender that sets a later flag must also set all earlier HMEM flags (i.e., the header always includes the previously defined fields).
...
| `HANDSHAKE_HMEM_P2P_HDR`      | $2^3$    | `0x0008` | `p2p_supported`  |
...
| `emitter_ope_type` | 4 | integer | `uint32_t` | the emitting side's operation type: 1 = the transfer's sender is aborting, 2 = the transfer's receiver is aborting |
...
transfer), the outcome depends on the state of the transaction at the
receiver.

If the transfer has already completed and a completion has been
generated, the PEER_ERROR packet is ignored.

If the transfer was never matched to a receive operation, all internal
state for it (including any data held in the provider's internal bounce
buffers) is discarded. No completion is generated.

If the transfer was matched to a receive operation, an error completion
is generated with `err = FI_ECANCELED` / `prov_errno =
FI_EFA_ERR_PEER_ABORTED` once any in-flight device work for it finishes.
There are no guarantees on how much data reached the application buffer.

If the message has not been seen at all, the abort is recorded so message
ordering can advance past a message ID that will never arrive, and a
buffered out-of-order segment is marked aborted and discarded when
ordering reaches it.

When the **sender of data** gets a PEER_ERROR (the receiver could not
complete the transfer, read-based protocols), the outcomes are: ignore it
