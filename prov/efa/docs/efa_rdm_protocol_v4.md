...
| 9  | Peer error packet       | extra feature | libfabric 2.x    | Section 4.10 |
...
cleanly abandon an in-flight two-sided long protocol when one side
voluntarily withdraws (for example by closing a memory registration
or tearing down its endpoint). It was introduced in libfabric 2.x
to address a class of issues where the EFA RDM provider previously
surfaced internal protocol failures as user-visible recv CQ errors
and leaked the peer's send-side state.
...
| `send_id` | 4 bytes | sender-side ope id, or `UINT32_MAX` sentinel |
| `recv_id` | 4 bytes | receiver-side ope id, or `UINT32_MAX` sentinel |
...
| reserved | 4 bytes | trailing padding to keep 8-byte alignment |

Whichever direction is "live" populates its id field; the other is
set to `UINT32_MAX` so the receive-side dispatcher can route the
packet unambiguously.
...
aborted" status (`REMOTE_ERROR_BAD_ADDRESS` or `REMOTE_ERROR_ABORT`)
and the peer advertises the feature, the receiver:

1. Returns the matched `peer_rxe` to the SRX (so the user's posted
   `fi_recv` survives and can match a future incoming message),
   or — when the opt-in
   `FI_OPT_EFA_SENDRECV_IN_ORDER_ALIGNED_128_BYTES` once-only
   guarantee is active and the recv buffer has been touched —
   writes a user CQ error.
2. Sends a `PEER_ERROR` packet to the sender with
   `send_id = txe_id_carried_in_RTM`, `recv_id = UINT32_MAX`,
   and `prov_errno = the_failing_status`.
...
2. Sends a `PEER_ERROR` packet with `send_id = UINT32_MAX`,
   `recv_id = txe->rx_id_from_CTS`, and the underlying
   `prov_errno`.
...
1. Looks up its `rxe` by `recv_id`.
2. Returns the matched `peer_rxe` to the SRX (or, in the
   buffer-touched case, writes a user CQ error).
3. Releases the `rxe`.
