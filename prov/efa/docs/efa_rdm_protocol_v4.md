...
either before the post by the MR generation check (returns `-FI_ECANCELED`
from the post path) or after the post by the NIC
...
fires on both `INVALID_LKEY` (after post, NIC) and `FI_ECANCELED`
(before post, MR generation check) since the emit decision lives in
`efa_rdm_txe_handle_error()`, which both the device-WR and the queued
before-post cancellation paths reach. A zero-delivery transfer no longer
relies on the `rxe_map` lookup missing: the `REF_MSG_ID_SKIP` packet is
a deliberate reorder-window abort marker, not a spurious emit.
...
(`efa_rdm_peer_queue_aborted_msg_marker()`), owes no completion and
reuses the existing out-of-order machinery:

1. If the window already advanced past `msg_id`: clean drop.
2. If a segment for `msg_id` is buffered out-of-order: mark it aborted in
...
3. Otherwise (the common never-arrived case): hold this `PEER_ERROR`
   ...
   and overflow-promotion path slide past the abort marker exactly as they
