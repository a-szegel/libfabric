 ...
 * (Group A does so internally; Group B is unhandled here for
 * simplicity — pass a buffer-not-touched rxe).
 */
...
void test_efa_rdm_pke_handle_tx_error_longread_bad_address_requeues(struct efa_resource **state)
{
	...
	run_longread_read_error(resource, rxe,
				EFA_IO_COMP_STATUS_REMOTE_ERROR_BAD_ADDRESS);
	/* Group A: peer_rxe must be back in the SRX msg_queue. */
	assert_false(slist_empty(&srx_ctx->msg_queue));
	ofi_genlock_unlock(srx_ctx->lock);
 ...
 *        Group A receiver-side abort handler must:
 ...
 * that points at a Group A rxe in the in-flight state, then calling
...
void test_efa_rdm_pke_handle_send_completion_peer_error_releases_rxe(struct efa_resource **state)
{
	...
	err_hdr->send_id = 0xdead;
	err_hdr->recv_id = EFA_RDM_PEER_ERROR_ID_SENTINEL;
 ...
 *
 * After Group A re-queue (with peer-support, so the rxe stays alive
...
/**
 * @brief Group B + peer supports PEER_ERROR_PKT: when the recv buffer
 *        has been touched (so the 128-byte once-only-delivery rule
 *        forces a CQ error rather than a re-queue), AND the peer
 *        advertises the feature, we still post a PEER_ERROR_PKT so
 *        the sender can clean up its txe.
 *
 * This is the four-way combination Group B + peer supports that
 * the other commit-6 tests don't cover.
 */
void test_efa_rdm_rxe_handle_peer_aborted_op_group_b_with_peer_support(struct efa_resource **state)
{
	struct efa_resource *resource = *state;
	struct efa_rdm_ep *ep;
	struct util_srx_ctx *srx_ctx;
	...
	rxe->state = EFA_RDM_RXE_MATCHED;
	rxe->peer_rxe = peer_rxe;
	rxe->bytes_received = 64;	/* buffer touched -> Group B */
	rxe->cq_entry.flags = FI_RECV | FI_MSG;
	rxe->cq_entry.op_context = (void *) 0xc3;
	rxe->tx_id = 0xbabe;
	...
	/* Group B: rxe transitions to OPE_ERR via efa_rdm_rxe_handle_error. */
	assert_int_equal(rxe->state, EFA_RDM_OPE_ERR);

	/* peer_rxe stays owned by the rxe (NOT re-queued) — Group B does
	 * not re-queue the SRX entry. */
	assert_ptr_equal(rxe->peer_rxe, peer_rxe);
	assert_true(slist_empty(&srx_ctx->msg_queue));

	/* Even in Group B, when the peer supports the feature we still
	 * emit a PEER_ERROR_PKT so the sender can reap its txe. */
	assert_true(rxe->internal_flags & EFA_RDM_RXE_PEER_ERROR_IN_FLIGHT);
	assert_int_equal(ep->efa_outstanding_tx_ops, outstanding_before + 1);
	ofi_genlock_unlock(srx_ctx->lock);
...
void test_efa_rdm_pke_handle_peer_error_recv_longcts_requeues_rxe(struct efa_resource **state)
{
	...
	err_hdr->version = EFA_RDM_PROTOCOL_VERSION;
	err_hdr->flags = EFA_RDM_PKT_CONNID_HDR;
	err_hdr->send_id = EFA_RDM_PEER_ERROR_ID_SENTINEL;
	err_hdr->recv_id = rxe->rx_id;
...
/**
 * @brief Malformed PEER_ERROR_PKT — both id fields are set to the
 *        sentinel — must be dropped without crashing or asserting.
 *
 * The dispatcher logs a warning and consumes the rx pkt. No CQ
 * entry is written and no ope state is touched.
 */
void test_efa_rdm_pke_handle_peer_error_recv_malformed_drops(struct efa_resource **state)
{
	struct efa_resource *resource = *state;
	struct efa_rdm_ep *ep;
	struct efa_rdm_pke *pkt_entry;
	...
	/* Must not crash, must not write a CQ entry. fi_cq_readerr
	 * returns -FI_EAGAIN since no error was written. */
	efa_rdm_pke_handle_peer_error_recv(pkt_entry);
