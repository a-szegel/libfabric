 ...
 * PEER_ERROR_PKT, clears the in-flight flag, and releases the rxe so
 * ASan sees no leak. Acquires the SRX lock itself; the caller must
 * NOT hold it. */
 ...
 *          2. Set EFA_RDM_RXE_PEER_ERROR_IN_FLIGHT on the rxe.
...
void test_efa_rdm_rxe_emit_peer_error_emits_pkt(struct efa_resource **state)
{
	...
	assert_true(rxe->internal_flags & EFA_RDM_RXE_PEER_ERROR_IN_FLIGHT);
 ...
 *   4. Assert peer_rxe is back in SRX, no TX pkt posted, no
 *      EFA_RDM_RXE_PEER_ERROR_IN_FLIGHT flag set.
 */
 ...
 *        EFA_RDM_RXE_PEER_ERROR_IN_FLIGHT and releases the rxe.
 *
 * We exercise the handler directly by hand-building a TX pkt_entry
 * that points at a re-queued rxe in the in-flight state, then calling
...
void test_efa_rdm_pke_handle_tx_error_peer_error_pkt_releases_rxe(struct efa_resource **state)
{
	 ...
	 * guard) and EFA_RDM_RXE_PEER_ERROR_IN_FLIGHT set. */
	rxe = efa_rdm_ep_alloc_rxe(ep, peer, ofi_op_msg);
	assert_non_null(rxe);
 ...
 *
 * Before the fix: the first failure ran recover+emit and set
 * EFA_RDM_RXE_PEER_ERROR_IN_FLIGHT; the second failure matched the
 * "PEER_ERROR_PKT async TX failure" branch on the IN_FLIGHT flag
 * alone (no packet-type discriminator) and called
 * efa_rdm_rxe_release_internal() while the actual PEER_ERROR_PKT and
 * any other READ WRs were still in flight, causing a use-after-free
 * when those completions later landed.
 *
 * After the fix: the dispatcher recognizes the rxe is already in
 * peer-abort recovery (EFA_RDM_RXE_PEER_ABORT_HANDLED) and treats
 * subsequent failing siblings as pure drains -- no re-recover, no
 * re-emit, no premature release. The rxe is freed only by
 * efa_rdm_rxe_release_peer_abort_if_drained() once every WR that
 * uses it as wr_id has drained.
 */
