 ...
 * locally, then notify the sender. Mirrors the efa_rdm_pke_handle_tx_error
 * dispatch so the emission tests exercise the real recover+emit flow. */
static void recover_then_emit_peer_error(struct efa_rdm_ope *rxe, int prov_errno)
{
	bool requeued = efa_rdm_rxe_recover_from_peer_abort(rxe, prov_errno);
	efa_rdm_rxe_emit_peer_error(rxe, prov_errno, requeued);
}
	...
	struct efa_resource **state)
{
	...
	/* Cleanup mirrors test_..._emits_peer_error_pkt: drain queued
	 * pkts, clear in-flight flag, release rxe so ASan is happy.
