...
/**
 * @brief Verify FI_EFA_RDM_MAX_TXE sizes the txe pool when the request fits
 *
...
void test_efa_rdm_ope_txe_pool_cap_from_env(void **state)
{
	...
	assert_true(16384 <= EFA_RDM_MAX_TXE_LIMIT);
...
/**
 * @brief Open an endpoint with a FI_EFA_RDM_MAX_TXE request, report the cap used
 */
...
/**
 * @brief Verify a FI_EFA_RDM_MAX_TXE that does not fit is adjusted, not refused
 *
...
void test_efa_rdm_txe_id_bits_split(void **state)
{
	...
	assert_true(EFA_RDM_DEFAULT_TXE_POOL_SIZE <= EFA_RDM_MAX_TXE_LIMIT);
	...
	assert_false(id & ((uint32_t) 1 << EFA_RDM_OPE_ID_BITS));
...
void test_efa_rdm_txe_id_rejects_stale_id(void **state)
{
	...
	assert_int_equal(reused->gen, (uint8_t) (stale_gen + 1));
 ...
 * PENDING and, with its
 * single WR already drained, emits a msg_id-only PEER_ERROR_PKT; the
 * receiver decides the outcome from its reorder-window state.
 *
...
void test_efa_rdm_txe_handle_error_longcts_prepost_cancel_emits_skip(void **state)
{
	...
	/* The consolidated emit decision marked the txe PENDING, signalled
	 * it msg_id-only, and emitted the PEER_ERROR_PKT, keeping the txe
	 * alive for that packet's completion. */
	assert_true(txe->internal_flags & EFA_RDM_OPE_PEER_ABORT_PENDING);
	assert_true(txe->internal_flags & EFA_RDM_PEER_ERROR_EMITTED_OR_SKIPPED);
	assert_int_equal(ep->efa_outstanding_tx_ops, outstanding_before + 1);
}
 ...
 * it is keyed by msg_id (it has no rx_id), so no op_id hint is
 * emitted.
 */
