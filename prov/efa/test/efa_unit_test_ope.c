...
void test_efa_rdm_txe_id_bits_split(void **state)
{
	...
	/* the two halves fit below the tag, with a bit to spare */
	assert_true(EFA_RDM_TXE_ID_INDEX_BITS + EFA_RDM_TXE_ID_GEN_BITS < 32);
	...
	assert_true(EFA_RDM_TXE_POOL_SIZE_DEFAULT <= EFA_RDM_TXE_POOL_MAX_CNT);
...
void test_efa_rdm_txe_id_rejects_bad_ids(void **state)
{
	...
	assert_null(efa_rdm_ep_live_txe_from_id(ep, efa_env.rdm_txe_pool_size - 1));
}
