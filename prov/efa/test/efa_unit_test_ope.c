...
static void recover_then_emit_peer_error(struct efa_rdm_ope *rxe, int prov_errno)
{
	 ...
	 * not re-run mark/emit (emit asserts !HANDLED). */
 ...
 */
void test_efa_rdm_rxe_peer_abort_writes_error_completion_at_drain(struct efa_resource **state)
{
	struct efa_resource *resource = *state;
	struct efa_rdm_ep *efa_rdm_ep;
	struct util_srx_ctx *srx_ctx;
 ...
 * that points at an rxe in the post-emit (PEER_ABORT_HANDLED) state,
...
void test_efa_rdm_pke_handle_tx_error_peer_error_pkt_releases_rxe(struct efa_resource **state)
{
	 ...
	 * state: matched and EFA_RDM_RXE_PEER_ABORT_HANDLED set. The user
 ...
 *
 * The first invocation sets EFA_RDM_RXE_PEER_ABORT_HANDLED; the
...
void test_efa_rdm_rxe_emit_peer_error_reentry_safe(struct efa_resource **state)
{
	...
	/* Second failure on the same rxe -- must early-return (HANDLED
	...
	struct efa_resource **state)
{
	 ...
	 * completion and reaped the rxe (no success entry). */
	assert_int_equal(fi_cq_read(resource->cq, NULL, 1), -FI_EAGAIN);
	assert_cq_peer_aborted_error(resource);
}
/**
 ...
 * WR marks the rxe peer-aborted (PEER_ABORT_HANDLED; the user error
	...
	struct efa_resource **state)
{
	...
	/* WR #1 fails (peer-abort) -> mark sets HANDLED. No peer feature
	 ...
	 * error completion was written at drain. */
