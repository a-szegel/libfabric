 ...
 * efa_rdm_peer_queue_aborted_msg_tombstone() helper directly, whereas
 ...
 * and is buffered (then tombstoned so the drain releases it without the
 * full recv-match/CQ path). Dispatching the SKIP packet for 0 tombstones
	...
	void **state)
{
	...
	/* msg_id 1 arrives OOO -> buffered in recvwin slot 1, then tombstoned
	 ...
	 * actual receive entry point), not the tombstone helper. The
	 * dispatcher routes REF_MSG_ID_SKIP to the tombstone path and
	 * consumes the packet. */
	...
	/* Window advanced past the aborted 0 and the tombstoned 1. */
	assert_true(ofi_recvwin_id_processed(&peer->robuf, 0));
	assert_true(ofi_recvwin_id_processed(&peer->robuf, 1));
	assert_int_equal((&peer->robuf)->exp_msg_id, 2);
