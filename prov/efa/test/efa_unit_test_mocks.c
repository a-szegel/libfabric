...
struct efa_rdm_ah g_dummy_rdm_ah = {0};
			    ...
			    size_t alloc_size)
{
	...
	if (g_ibv_ah_cnt < g_self_ah_cnt) {
		...
		g_dummy_rdm_ah.ah.ibv_ah = &g_dummy_ah;
		g_dummy_rdm_ah.ah.ahn = -1;
		memset(g_dummy_rdm_ah.ah.gid, 0, sizeof(g_dummy_rdm_ah.ah.gid));
		g_dummy_rdm_ah.ah.refcnt = 1;
		/*
		 * Reset protocol fields so efa_rdm_ah_alloc sees a fresh AH
		 ...
		 */
		g_dummy_rdm_ah.implicit_refcnt = 0;
		g_dummy_rdm_ah.explicit_refcnt = 0;
		memset(&g_dummy_rdm_ah.lru_list_entry, 0,
		       sizeof(g_dummy_rdm_ah.lru_list_entry));
		dlist_init(&g_dummy_rdm_ah.implicit_conn_list);
		return &g_dummy_rdm_ah.ah;
	} else {
