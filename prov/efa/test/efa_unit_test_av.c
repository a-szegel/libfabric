...
static void efa_ah_cnt_av_efa_impl(struct efa_resource **state, bool multi_av)
{
	...
	assert_int_equal(HASH_CNT(hh, efa_domain->ah_map), 1);
	assert_int_equal(efa_rdm_ah_from_ah(efa_ah)->explicit_refcnt, 3);
	assert_int_equal(efa_rdm_ah_from_ah(efa_ah)->implicit_refcnt, 0);
...
static void test_av_verify_av_hash_cnt(struct efa_av *av, struct efa_rdm_av *rdm_av,
...
void test_av_reinsertion(struct efa_resource **state)
{
	...
	test_av_verify_av_hash_cnt(av, rdm_av, 1, 0, 0, 0);
	...
	assert_int_equal(err, 0);
	test_av_verify_av_hash_cnt(av, rdm_av, 0, 0, 0, 0);
...
void test_av_reverse_av_remove_qpn_collision(struct efa_resource **state)
{
	...
	test_av_verify_av_hash_cnt(av, rdm_av, 1, 1, 0, 0);
...
static struct efa_rdm_peer *test_av_get_peer_from_implicit_av(struct efa_resource *resource)
{
	...
	err = efa_rdm_av_insert_one(rdm_av, &raw_addr, &implicit_fi_addr, 0, NULL, true, true);
	...
	test_addr = efa_rdm_av_reverse_lookup_implicit(rdm_av, ahn, raw_addr.qpn, NULL);
...
void test_av_implicit_to_explicit(struct efa_resource **state)
{
	...
	ahn = efa_rdm_ep->self_ah->ahn;
	test_addr = efa_rdm_av_reverse_lookup(rdm_av, ahn, raw_addr.qpn, NULL);
...
static void test_av_implicit_av_verify_lru_list_first_last_elements(
	struct efa_rdm_av *rdm_av, struct efa_rdm_av_entry *first_entry_expected,
	struct efa_rdm_av_entry *last_entry_expected)
{
	struct dlist_entry *first_entry, *last_entry;
	struct efa_rdm_av_entry *first_entry_actual, *last_entry_actual;

	first_entry = rdm_av->implicit_av_lru_list.next;
	last_entry = rdm_av->implicit_av_lru_list.prev;

	first_entry_actual = container_of(first_entry, struct efa_rdm_av_entry,
					 implicit_av_lru_entry);
	last_entry_actual = container_of(last_entry, struct efa_rdm_av_entry,
					implicit_av_lru_entry);

	assert_ptr_equal(first_entry_actual, first_entry_expected);
	assert_ptr_equal(last_entry_actual, last_entry_expected);
}
...
void test_av_implicit_av_lru_insertion(struct efa_resource **state)
{
	...
	test_av_verify_av_hash_cnt(av, rdm_av, 0, 0, 1, 0);

	/* Expected LRU list: HEAD->peer0 */
	test_av_implicit_av_verify_lru_list_first_last_elements(rdm_av, peer0->av_entry, peer0->av_entry);

	/* Manually insert second address into implicit AV */
	peer1 = test_av_get_peer_from_implicit_av(resource);
	test_av_verify_av_hash_cnt(av, rdm_av, 0, 0, 2, 0);

	/* Expected LRU list: HEAD->peer0->peer1 */
	test_av_implicit_av_verify_lru_list_first_last_elements(rdm_av, peer0->av_entry, peer1->av_entry);

	/* Manually insert third address into implicit AV */
	peer2 = test_av_get_peer_from_implicit_av(resource);
	test_av_verify_av_hash_cnt(av, rdm_av, 0, 0, 3, 0);

	/* Expected LRU list: HEAD->peer0->peer1->peer2 */
	test_av_implicit_av_verify_lru_list_first_last_elements(rdm_av, peer0->av_entry, peer2->av_entry);
	...
	implicit_fi_addr = efa_rdm_av_reverse_lookup_implicit(
		rdm_av, ahn, efa_rdm_av_entry_ep_addr(peer0->av_entry)->qpn, NULL);
	ofi_genlock_unlock(&efa_rdm_ep->base_ep.domain->srx_lock);
	assert_int_equal(implicit_fi_addr, 0);

	/* Expected LRU list: HEAD->peer1->peer2->peer0 */
	test_av_implicit_av_verify_lru_list_first_last_elements(rdm_av, peer1->av_entry, peer0->av_entry);

	/* Access peer2 through the CQ read path */
	ahn = efa_rdm_ep->self_ah->ahn;
	ofi_genlock_lock(&efa_rdm_ep->base_ep.domain->srx_lock);
	implicit_fi_addr = efa_rdm_av_reverse_lookup_implicit(
		rdm_av, ahn, efa_rdm_av_entry_ep_addr(peer2->av_entry)->qpn, NULL);
	ofi_genlock_unlock(&efa_rdm_ep->base_ep.domain->srx_lock);
	assert_int_equal(implicit_fi_addr, 2);

	/* Expected LRU list: HEAD->peer1->peer0->peer2 */
	test_av_implicit_av_verify_lru_list_first_last_elements(rdm_av, peer1->av_entry, peer2->av_entry);


	/* Access peer1 through repeated AV insertion path */
	ofi_genlock_lock(&efa_rdm_ep->base_ep.domain->srx_lock);
	err = efa_rdm_av_insert_one(rdm_av, efa_rdm_av_entry_ep_addr(peer1->av_entry), &implicit_fi_addr, 0, NULL, true, true);
	ofi_genlock_unlock(&efa_rdm_ep->base_ep.domain->srx_lock);
	assert_int_equal(err, 0);
	...
	ofi_genlock_lock(&efa_rdm_ep->base_ep.domain->srx_lock);
	err = efa_rdm_av_insert_one(rdm_av, efa_rdm_av_entry_ep_addr(peer2->av_entry), &implicit_fi_addr, 0, NULL, true, true);
	ofi_genlock_unlock(&efa_rdm_ep->base_ep.domain->srx_lock);
	assert_int_equal(err, 0);
	assert_int_equal(implicit_fi_addr, 2);
	test_av_verify_av_hash_cnt(av, rdm_av, 0, 0, 3, 0);
...
void test_av_implicit_av_lru_eviction(struct efa_resource **state)
{
	 ...
	 * and peer->av_entry becomes stale memory (entry->ep_addr is zeroed
	 * by efa_rdm_av_entry_release_util_av).
	 */
	...
	assert_int_equal(HASH_CNT(hh, rdm_av->evicted_peers_hashset), 1);
	HASH_FIND(hh, rdm_av->evicted_peers_hashset, &peer1_ep_addr,
	...
	ofi_genlock_lock(&efa_rdm_ep->base_ep.domain->srx_lock);
	err = efa_rdm_av_insert_one(rdm_av, efa_rdm_av_entry_ep_addr(peer0->av_entry), &implicit_fi_addr, 0, NULL, true, true);
	ofi_genlock_unlock(&efa_rdm_ep->base_ep.domain->srx_lock);
	assert_int_equal(err, 0);
	assert_int_equal(implicit_fi_addr, 0);
	test_av_verify_av_hash_cnt(av, rdm_av, 0, 0, 2, 0);
	...
	test_av_implicit_av_verify_lru_list_first_last_elements(rdm_av, peer2->av_entry, peer0->av_entry);
	...
	assert_int_equal(HASH_CNT(hh, rdm_av->evicted_peers_hashset), 2);
	HASH_FIND(hh, rdm_av->evicted_peers_hashset, &peer2_ep_addr,
		  sizeof(struct efa_ep_addr), efa_ep_addr_hashable);
	assert_non_null(efa_ep_addr_hashable);
	...
	test_av_implicit_av_verify_lru_list_first_last_elements(rdm_av, peer0->av_entry, peer3->av_entry);
}
...
void test_ah_refcnt(struct efa_resource **state)
{
	...
	/* Manually insert into implicit AV */
	ofi_genlock_lock(&efa_rdm_ep->base_ep.domain->srx_lock);
	err = efa_rdm_av_insert_one(rdm_av, &raw_addr, &fi_addr, 0, NULL, true, true);
...
void test_ah_lru_eviction_impl(bool explicit)
{
	...
	rdm_av = container_of(efa_av[0], struct efa_rdm_av, efa_av);
	...
	err = efa_rdm_av_insert_one(rdm_av, &raw_addr[0], &fi_addr, 0, NULL, true, true);
	...
	if (explicit) {
		...
		ofi_genlock_lock(&efa_rdm_ep[0]->base_ep.domain->srx_lock);
		err = efa_rdm_av_insert_one(rdm_av, &raw_addr[1], &fi_addr, 0, NULL, true, true);
		peer = efa_rdm_ep_get_peer_implicit(efa_rdm_ep[0], fi_addr);
		ofi_genlock_unlock(&efa_rdm_ep[0]->base_ep.domain->srx_lock);
	}
