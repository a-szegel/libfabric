// OPAL_FREE_LIST_T - the things Seth doesn't think we need
...
struct opal_free_list_t {
    ...
    /** Size, Assume List is static configurable (doesn't grow for now) */
    size_t list_size;

    /** Assume all fragments are the same class, hard coded in for now */
    /** Assume that alignment does actually matter*/
    /** Assume that all access to the list is single threaded*/
    /** Need some way for initial allocation */
